package com.altes.alphacure.staff.security;
    ...
    public Collection<GrantedAuthority> convert(Jwt jwt) {
        ...
        if (resourceAccess != null) {
            for (Map.Entry<String, Object> entry : resourceAccess.entrySet()) {
                if (entry.getValue() instanceof Map<?, ?> ca && ca.get("roles") instanceof List<?> roles) {
                    roles.forEach(r -> authorities.add(
                            new SimpleGrantedAuthority("ROLE_" + entry.getKey() + "_" + r)));
                }
            }
        }
        return authorities;
